...
###################################################################
flog.debug("Dataframe with expressed genes in the given condition")
###################################################################

sfx <- conditions
res.up <- genes.up[[1]]

for(i in head(seq_along(genes.up), -1)) {
  res.up <- merge(res.up, genes.up[[i+1]], all = TRUE, 
                  suffixes = sfx[i:(i+1)], by = "REFSEQ")
}

res.up <- res.up[,1]
res.up <- as.vector(res.up)

res.down <- genes.down[[1]]

for(i in head(seq_along(genes.down), -1)) {
  res.down <- merge(res.down, genes.down[[i+1]], all = TRUE, 
                    suffixes = sfx[i:(i+1)], by = "REFSEQ")
}

res.down <- res.down[,1]
res.down <- as.vector(res.down)
...
saveRDS(TPM_HM, file.path(proj.dir, data.dir, "DF.TPM_normalization.RDS"))


flog.debug("Heatmap for upregulated genes")

# laod data
TPM_HM = readRDS(file.path(proj.dir, data.dir, "DF.TPM_normalization.RDS"))

# filter data to select upregulated genes
HeatmapData <- filter(TPM_HM, RefSeq %in% res.up)
...
heatmap.generator <- function(
  ...
  list.of.BP = BP.up.sim.cutoff, # dataframe with biological process
...
heatmap.generator(dataset = DF, TPM_HM = TPM_HM, list.of.BP = BP.up.sim.cutoff, 
